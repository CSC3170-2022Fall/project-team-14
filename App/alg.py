...
def change_start_call(machine_id, start_time, operation_type):
    ...
    time_queue.put([cur_time+start_time,2,machine_id,operation_type])
...
def handle(next_exe):
    ...
    if(next_exe[1] == 1): #package exe next operation
        ...
        if sum >= package_info[0]['chip_number']:
            ...
            for i in range(len(machine_info)):
                ...
                time_queue.put([actual_end_time_machine,4,machine_info[i]['machine_id']],package_id,next_operation_type)
    ...
    if(next_exe[1] == 4):#machine end
        ...
        cursor.execute("UPDATE Process_record set Process_record.end_time = %s WHERE (Process_record.package_id = %s and Process_record.operation_type = %s and Process_record.machine_id = %s)",cur_time, next_exe[3],next_exe[4],next_exe[2])
